    ...
    usage(FILE * out)
{
	...
	fputs(USAGE_OPTIONS_P, out);
	fputs(" -b, --bytes         show output in bytes\n", out);
	fputs("     --kilo          show output in kilobytes\n", out);
	fputs("     --mega          show output in megabytes\n", out);
	fputs("     --giga          show output in gigabytes\n", out);
	fputs("     --tera          show output in terabytes\n", out);
	fputs("     --peta          show output in petabytes\n", out);
	fputs(" -k, --kibi          show output in kibibytes\n", out);
	fputs(" -m, --mebi          show output in mebibytes\n", out);
	fputs(" -g, --gibi          show output in gibibytes\n", out);
	fputs("     --tebi          show output in tebibytes\n", out);
	fputs("     --pebi          show output in pebibytes\n", out);
	fputs(" -h, --human         show human-readable output\n", out);
	fputs("     --si            use powers of 1000 not 1024\n", out);
	fputs(" -l, --lohi          show detailed low and high memory statistics\n", out);
	fputs(" -t, --total         show total for RAM + swap\n", out);
	fputs(" -s N, --seconds N   repeat printing every N seconds\n", out);
	fputs(" -c N, --count N     repeat printing N times, then exit\n", out);
	fputs(" -w, --wide          wide output\n", out);
	fputs(USAGE_SEPARATOR_P, out);
	fputs("     --help     display this help and exit\n", out);
	fputs(USAGE_VERSION_P, out);
...
int main(int argc, char **argv)
{
		...
		case 'c':
			...
			if (args.repeat_counter < 1)
			  error(EXIT_FAILURE, ERANGE,
				  "failed to parse count argument: '%s'", optarg);
			break;
		case 'w':
			flags |= FREE_WIDE;
			break;
	...
	do {
		...
		printf("total    : %11s\n", scale_size(kb_main_total, flags, args));
		printf("used     : %11s\n", scale_size(kb_main_used, flags, args));
		printf("free     : %11s\n", scale_size(kb_main_free, flags, args));
		printf("shared   : %11s\n", scale_size(kb_main_shared, flags, args));
		printf("buffers  : %11s\n", scale_size(kb_main_buffers, flags, args));
		printf("cached   : %11s\n", scale_size(kb_main_cached, flags, args));
		printf("available: %11s\n", scale_size(kb_main_available, flags, args));
		/*
