...
be omitted.  Instead B<-o log> you can write B<-olog>.
...
Note that a combination with B<-k> is only well-defined for
...
``time'' of this timeout refers I<idle time>: if, at any point in
...
=item S<Mozilla and Netscape 6.x.>
...
to log file F<log>.  It is tiring to type B<--tries>, so we
shall use B<-t>.
...
background.  To unlimit the number of retries, use B<-t inf>.
...
If you specify B<-> as file name, the URLs will be read from
standard input.
...
document, saving the log of the activities to F<gnulog>:
...
The HTML page will be saved to F<www.server.com/dir/page.html>, and
the images, stylesheets, etc., somewhere under F<www.server.com/>,
depending on where they were on the remote server.


=item *

The same as the above, but without the F<www.server.com/> directory.
In fact, I don't want to have all those random server directories
anyway---just save I<all> those files under a F<download/>
...
Retrieve the index.html of B<www.lycos.com>, showing the original
server headers:
...
Retrieve the first two levels of B<wuarchive.wustl.edu>, saving them
to F</tmp>.
...
server.  You tried B<wget http://www.server.com/dir/*.gif>, but that
...
More verbose, but the effect is the same.  B<-r -l1> means to
retrieve recursively, with maximum depth
of 1.  B<--no-parent> means that references to the parent directory
are ignored, and B<-A.gif> means to
download only the GIF files.  B<-A "*.gif"> would have worked
too.
...
subdirectories), use B<--mirror> (B<-m>), which is the shorthand
for B<-r -l inf -N>.  You can put Wget in the crontab file asking it
...
when HTML files are saved under extensions other than B<.html>,
perhaps because they were served as F<index.cgi>.  So you'd like
Wget to rename all the files served with content-type B<text/html>
or B<application/xhtml+xml> to F<I<name>.html>.
